...
7. Fix only after the cause is supported by evidence.
8. Add or describe regression coverage at the right public interface, then rerun the original feedback loop.
9. Clean up temporary probes and report the root cause, fix, verification, residual risk, and any architecture follow-up.
...
- Use `policy-testing` for verification depth when test strategy is the central question.
