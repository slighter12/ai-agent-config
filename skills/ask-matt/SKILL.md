...
Read [the installed catalog](references/CATALOG.md) when exact availability, invocation, or local-extension status matters.
...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff. A commit remains a separate explicitly authorized action. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
...
- **`/resolving-merge-conflicts`** — work an in-progress merge or rebase conflict hunk by hunk, resolving by **intent** traced to each side's primary source rather than by picking lines, then stage the resolved paths and stop. Finish or continue the merge/rebase only when the user explicitly requests it; never run `--abort`. Standalone and off every flow: reach for it when you are already mid-conflict.
- **`/prototype`** — a small, throwaway program that answers one design question: does this state model feel right, or what should this UI look like. Throwaway is a constraint on how the code is written, not a promise to destroy it: the answer folds into the real code, and, when branch and commit creation are authorized, the prototype itself is kept as a **primary source** on a `prototype/<name>` branch out of main. It's the detour in step 2 of the main flow, but reach for it any time a design question is hard to settle on paper.
- **`/research`** — delegate reading legwork to a provider-native **background agent** by default: it investigates a question against **primary sources**, then returns one cited Markdown result while you keep working. If that primitive is unavailable or its launch fails, complete the same pass in the current session without dropping any research requirement. Save the result in the repo only when artifact creation is authorized; research feeds the thinking, it doesn't replace it.
