...
- The user asks to build an approved throwaway prototype after the prototype question and scope are clear.

## Avoid When

- The failure is not understood or reproducible; use `diagnose`.
- The user only wants to decide whether a prototype is needed; use `planning-grill`.
...
4. For an approved throwaway prototype, state the prototype question, keep the artifact deliberately scoped and throwaway, provide one run command or path when project tooling supports it, default to no persistence, surface the relevant state or output the prototype is meant to inspect, and plan a disposition: delete, absorb into production work, capture candidate, or pending user decision.
5. Check cross-cutting risks while implementing:
...
6. Before finalizing the approach, load the corresponding `policy-*` skill when that policy area materially affects correctness, safety, or acceptance. This is a hard gate for API contracts, security/auth/secrets/PII, verification strategy, runtime/deploy/config behavior, and language/framework ownership boundaries. Do not load unrelated policy skills for routine edits where the built-in guardrails fully cover the risk.
7. If implementation reveals a product/architecture decision, rejected approach, deferred scope,
...
- Do not let throwaway prototype code become production scaffolding by accident; name or describe it as throwaway and report its disposition.
- For throwaway prototypes, do not add persistence, storage, new dependencies, production abstractions, or broad scaffolding unless explicitly approved. If persistence is the question being tested, use scratch/local storage with a clear wipe-me marker.
