...
const GitWorkflowReminder = "This user prompt matched a direct simple branch, commit, push, or PR action. Start multi-agent spawn discovery/launch and the read-only `conventional-git-flow` Git Context Pack in parallel when tooling allows: `git status --short --branch`, `git diff --name-status`, `git diff --stat`, `git diff --cached --name-status`, `git diff --cached --stat`, and `git log -5 --oneline`; then inspect intended files with narrow `git diff -- <paths...>` commands. Prefer the `git-commit` role with a compact handoff; include context pack results if they are already available, but do not wait on parent preflight if the delegate can run it. If no spawn_agent/start-agent tool is visible, search/discover the multi-agent spawn tool. If no callable spawn route exists after discovery, continue in the main session using the same context pack and safety rules, and report that fallback. Once delegation starts, the main session must not run mutating git commands. Do not use broad staging, amend, rebase, force push, `--no-verify`, or push/PR unless the user requested that action."
